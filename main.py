...
# --- Configuration ---
TOKEN = os.getenv('TELEGRAM_BOT_TOKEN') # Render থেকে টোকেন নেওয়া হবে
OWNER_ID = int(os.getenv('OWNER_ID')) # Render থেকে আইডি নেওয়া হবে
ADMIN_ID = int(os.getenv('ADMIN_ID')) # Render থেকে আইডি নেওয়া হবে
    ...
    try:
        ...
        if not lang_info.get('executable', True):
            ...
            if message_for_updates:
                ...
                if render_url:
                    file_url = f"{render_url}/file/{file_hash}"
                else:
                    file_url = "URL will be available after deployment."
    ...
    for i, (file_name, file_type) in enumerate(files, 1):
        if file_type == 'executable':
            ...
            if render_url:
                ...
                file_url = "URL not available yet."
...
if __name__ == "__main__":
    if not os.getenv('TELEGRAM_BOT_TOKEN') or not os.getenv('OWNER_ID'):
        print("ERROR: TELEGRAM_BOT_TOKEN and OWNER_ID must be set in the environment.")
        sys.exit(1)
